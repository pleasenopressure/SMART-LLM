...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="train_task_allocation",
        choices=["train_task_allocation"],
    )

    parser.add_argument(
        "--test-set", type=str, default="final_test", choices=["final_test"]
    )
    ...
    decompose_prompt_file = open(
        os.getcwd() + "/data/pythonic_plans/" + args.prompt_decompse_set + ".py", "r"
    )
    ...
    for task in test_tasks:
        ...
        if "gpt" not in args.gpt_version:
            ...
            _, text = LM(
                messages, args.gpt_version, max_tokens=1300, frequency_penalty=0.0
            )
    ...
    for i, plan in enumerate(decomposed_plan):
        ...
        if "gpt" not in args.gpt_version:
            ...
            messages = [{"role": "user", "content": curr_prompt}]
            _, text = LM(
                messages, args.gpt_version, max_tokens=1500, frequency_penalty=0.35
            )
            ...
            _, text = LM(
                messages, args.gpt_version, max_tokens=400, frequency_penalty=0.69
            )
    ...
    prompt_file1 = (
        os.getcwd() + "/data/pythonic_plans/" + args.prompt_allocation_set + "_code.py"
    )
    ...
    for i, (plan, solution) in enumerate(zip(decomposed_plan, allocated_plan)):
        ...
        if "gpt" not in args.gpt_version:
            ...
            _, text = LM(
                messages, args.gpt_version, max_tokens=1400, frequency_penalty=0.4
            )
